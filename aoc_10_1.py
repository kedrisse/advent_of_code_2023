def find_farthest_point(grid):
    ...
    def is_valid(y, x, y_direction, x_direction):
        if not (0 <= y + y_direction < rows and 0 <= x + x_direction < cols):
            return False
        if (
            y_direction == -1
            and x_direction == 0
            and grid[y + y_direction][x + x_direction] in "|7F"
        ):
            return True
        elif (
            y_direction == 1
            and x_direction == 0
            and grid[y + y_direction][x + x_direction] in "|JL"
        ):
            return True
        elif (
            y_direction == 0
            and x_direction == 1
            and grid[y + y_direction][x + x_direction] in "-7J"
        ):
            return True
        elif (
            y_direction == 0
            and x_direction == -1
            and grid[y + y_direction][x + x_direction] in "-FL"
        ):
            return True
        print(
            f"({y_direction},{x_direction}) : {grid[y + y_direction][x + x_direction]}"
        )
        return False
    ...
    def update_direction(current_direction, turn_direction):
        ...
        return turn_mapping[turn_direction][current_direction]

    # Find the starting position
    current_y, current_x = None, None
    for i in range(rows):
        for j in range(cols):
            if grid[i][j] == "S":
    ...
    for direction in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
        if is_valid(current_y, current_x, direction[0], direction[1]):
    ...
    while grid[current_y][current_x] != "S":
        ...
        prev_direction = update_direction(prev_direction, current_pipe)
        current_y += prev_direction[0]
        current_x += prev_direction[1]
        visited.append((current_y, current_x))

    return len(visited) // 2


grid = """|J.7F7-L--77F7-J-F-77FF-7FLLJ7F-J-F---7-J-L-FJ-FJFJ7-7-FF--L7.L--|.FJ-J77FFF|77-7F-7-FFL7-|7-F7-L7F7FF7.|----LL-77FF.F--F-J-LFFL-F-LF7--F-FL
...
LJ.LF-FJ-L7-7JJL--F7-LFJ-L.7.L-J-|-L--.F7LJ---JJJ-L-FLLJ-.L|J|.L.L7-L7-F-LJLJ-|-|L-|--JJ|LJ-L-JFJL-JJL7-|LL7JJJL-LLFLFLJLJJLL-J-JL-.LLF-J--J""".split(
    "\n"
)
