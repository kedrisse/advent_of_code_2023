def is_possible_game(cube_counts):
...
def possible_games_sum(game_records):
    powers = []

    for game_record in game_records:
        game_info = game_record.split(": ")[1].split("; ")
        cube_max = {"red": 0, "green": 0, "blue": 0}

        for info in game_info:
            for cube_color in info.split(", "):
                parts = cube_color.split()
                count = int(parts[0])
                color = parts[1]

                cube_max[color] = count if count > cube_max[color] else cube_max[color]
...
game_records = """Game 1: 2 red, 2 green; 6 red, 3 green; 2 red, 1 green, 2 blue; 1 red
...
Game 100: 8 red, 2 blue, 1 green; 2 blue, 4 red, 2 green; 9 red, 1 green; 2 green, 2 red; 3 red, 5 blue; 5 blue, 8 red""".split(
    "\n"
)

# Calculate the sum of IDs for possible games
result = possible_games_sum(game_records)
