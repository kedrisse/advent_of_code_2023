def update_gears(engine_schematic, i, rows, j, end_num, cols, gears):
    for di in range(max(i - 1, 0), min(i + 2, rows)):
        for dj in range(max(j - 1, 0), min(end_num + 2, cols)):
            if engine_schematic[di][dj] == "*":
                gear_pos = f"{di},{dj}"
                if gear_pos not in gears:
                    gears[gear_pos] = [int(engine_schematic[i][j : end_num + 1])]
                else:
                    gears[gear_pos].append(int(engine_schematic[i][j : end_num + 1]))
    ...
    for i in range(rows):
        ...
        while j < cols:
            ...
            if char.isdigit():
                ...
                update_gears(engine_schematic, i, rows, j, end_num, cols, gears)
                j = end_num + 1
            j += 1
...
result = calculate_gear_ratios(engine_schematic)

# Print the result
print("The sum of all gear rations in the engine schematic is:", result)
